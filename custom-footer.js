$(function () {
  ...
  function addIncidentTypeToIncidentList ($list, pageType) {
    const $incidentLinks = $list.querySelectorAll('a')

    if ($incidentLinks.length > 0) {
      $incidentLinks.forEach($el => addIncidentTypeToTarget($el, pageType, 'after'))
    }
  }
  ...
  if ($container !== null) {
    ...
    if (pathRoot === '/') {
      ...
      if (($aboutText !== null) && ($pageStatus !== null) && ($pageStatusContent !== null)) {
        ...
        addIncidentTypeToIncidentList($incidentsList, 'rootPage')
    ...
    if (pathRoot === '/incidents') {
      ...
      addIncidentTypeToTarget($incidentHeading, 'incidentPage')
    }
  }
})
